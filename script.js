...
function updateNextButton() {
  ...
  if (isEnabled) {
    nextBtn.classList.remove('opacity-50', 'bg-white/10', 'text-white/30');
    nextBtn.classList.add('bg-white', 'text-black', 'hover:scale-[1.03]', 'active:scale-[0.98]');
    nextBtn.disabled = false;
  } else {
    nextBtn.classList.add('opacity-50', 'bg-white/10', 'text-white/30');
...
function showInspiration(data) {
  ...
  setTimeout(() => {
    ...
    optionsEl.innerHTML = `
      <div class="inspiration-container flex flex-col items-center text-center">
        <img src="${data.image}" class="inspiration-img step-${data.step}-img" alt="Inspiration">
        <h2 class="text-[32px] font-extrabold leading-tight mb-4 tracking-tighter">${data.title || "Progresso!"}</h2>
        <p class="inspiration-text">${data.text}</p>
      </div>
    `;
  ...
  mainContent.innerHTML = `
    <div class="flex flex-col items-center">
      ...
      <div class="text-center mb-10">
        ...
        <p class="text-white/40">Preparando seu diagnóstico...</p>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4 mb-10 w-full">
        ...
        <div class="bg-white/5 border border-white/10 rounded-2xl p-4">
          <span class="text-[10px] text-white/30 uppercase tracking-widest block mb-2">Base</span>
          <div class="flex items-center gap-2 text-sm font-bold">
            <div class="w-1.5 h-1.5 bg-primary rounded-full shadow-[0_0_8px_#38BDF8]"></div> 
            +50k perfis
          </div>
        ...
        <div class="bg-white/5 border border-white/10 rounded-2xl p-4">
          <span class="text-[10px] text-white/30 uppercase tracking-widest block mb-2">Precisão</span>
          <div class="flex items-center gap-2 text-sm font-bold">
            <div class="w-1.5 h-1.5 bg-primary rounded-full shadow-[0_0_8px_#38BDF8]"></div> 
            94.7%
          </div>
        ...
        <div class="bg-white/5 border border-white/10 rounded-2xl p-4">
          <span class="text-[10px] text-white/30 uppercase tracking-widest block mb-2">Módulos</span>
      ...
      <div class="bg-white/5 border border-white/10 rounded-3xl p-8 w-full">
        <div class="flex justify-between mb-6 relative">
          ${[1, 2, 3, 4, 5].map(i => `
            <div class="flex flex-col items-center gap-2 z-10" id="step-node-${i}">
              ...
              <span class="text-[9px] text-white/30 font-bold">${i}/5</span>
            </div>
          `).join('')}
        </div>
        ...
        <div class="flex justify-between text-[10px] font-bold text-white/30 uppercase tracking-widest">
          <span id="load-status-text">Processando</span>
          <span class="text-accent" id="load-pct-text">0%</span>
        </div>
      </div>

      <div class="mt-10 text-white/30 text-[10px] tracking-widest uppercase text-center font-mono">
    ...
    <div class="flex flex-col items-center text-center">
      ...
      <div class="relative w-56 h-56 mb-12 flex items-center justify-center">
        ...
        <div class="absolute flex flex-col items-center">
          <div class="text-6xl font-black tracking-tighter"><span id="score-counter">0.0</span><span class="text-2xl text-white/30">/10</span></div>
          <p class="text-[10px] uppercase font-bold tracking-widest text-white/40 mt-1">Score de Performance</p>
        </div>
      </div>
        ...
        ${[
          ...
          <div class="text-left">
            <div class="flex justify-between items-end mb-2">
              <span class="text-xs font-bold text-white/50 uppercase tracking-widest">${s.label}</span>
      ...
      <div class="text-[10px] font-mono text-white/20 uppercase tracking-widest">
      ...
      <div class="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 w-full shadow-2xl relative overflow-hidden">
        ...
        <div class="mb-10">
          <div class="flex justify-between items-center mb-2">
            <span class="text-[11px] font-black uppercase tracking-widest text-white/30">Vagas remanescentes</span>
        ...
        <button class="w-full py-5 rounded-pill bg-primary text-black font-black text-xl hover:scale-[1.03] active:scale-[0.98] transition-all shadow-[0_20px_40px_rgba(0,224,255,0.2)]" onclick="window.location.href = window.apex_checkout_url || 'https://lastlink.com/p/CAA303628/checkout-payment/'">
          ADQUIRIR AGORA >
        </button>
        ...
        <div class="grid grid-cols-3 gap-4 mt-10">
          ${[
            ...
            <div class="flex flex-col items-center gap-2">
              <i data-lucide="${badge.icon}" class="w-5 h-5 text-white/20"></i>
              <span class="text-[9px] font-bold text-white/30 uppercase tracking-widest">${badge.text}</span>
            </div>
          `).join('')}
        </div>
      </div>

      <div class="mt-16 w-full max-w-[600px] mb-8">
        <div class="flex justify-around items-center border-y border-white/10 py-6 mb-10">
          <div class="text-center">
            ...
            <div class="text-[10px] text-white/50 uppercase tracking-widest font-bold">Avaliação Média</div>
          </div>
          ...
          <div class="text-center">
            ...
            <div class="text-[10px] text-white/50 uppercase tracking-widest font-bold">Homens Ativos</div>
          </div>
        </div>
        
        <h3 class="text-xl font-black text-center mb-8">Histórias de Sucesso</h3>
        ...
        <div class="space-y-4">
          ...
          <div class="bg-white/5 border border-white/10 p-6 rounded-2xl relative">
            <div class="flex justify-between items-start mb-3">
              ...
              <span class="text-[10px] font-bold text-white/30 truncate max-w-[100px]">Carlos F.</span>
            </div>
            <h4 class="font-bold text-sm mb-2">Mudou minha realidade</h4>
            <p class="text-white/60 text-xs leading-relaxed">Eu estava cético. Tentei suplementos e nunca funcionou. Com esse protocolo, em 3 semanas já notei a diferença. Acordo revigorado, com energia e uma rigidez que não via há anos. Melhor investimento que já fiz.</p>
          </div>
          ...
          <div class="bg-white/5 border border-white/10 p-6 rounded-2xl relative">
            <div class="flex justify-between items-start mb-3">
              ...
              <span class="text-[10px] font-bold text-white/30 truncate max-w-[100px]">Ricardo T.</span>
            </div>
            <h4 class="font-bold text-sm mb-2">Excelente controle</h4>
            <p class="text-white/60 text-xs leading-relaxed">Prático e direto ao ponto. Estava perdendo a confiança no relacionamento, mas o método me deu o controle de volta. Totalmente diferente de tudo que já vi na internet.</p>
          </div>
          ...
          <div class="bg-white/5 border border-white/10 p-6 rounded-2xl relative">
            <div class="flex justify-between items-start mb-3">
              <div class="flex text-[#f59e0b]">
                <i data-lucide="star" class="w-3 h-3 fill-current"></i><i data-lucide="star" class="w-3 h-3 fill-current"></i><i data-lucide="star" class="w-3 h-3 fill-current"></i><i data-lucide="star" class="w-3 h-3 fill-current"></i><i data-lucide="star" class="w-3 h-3 text-white/20 fill-transparent p-[0.5px]"></i>
              </div>
              <span class="text-[10px] font-bold text-white/30 truncate max-w-[100px]">Marcelo P.</span>
            </div>
            <h4 class="font-bold text-sm mb-2">Poucas semanas e já vejo resultados</h4>
            <p class="text-white/60 text-xs leading-relaxed">Estou na terceira semana e já percebo um fluxo muito melhor e uma enorme redução na ansiedade de terminar rápido. Recomendo muito, basta ter paciência e seguir o plano.</p>
          </div>
        </div>
      </div>

      <div class="mt-8 text-[10px] font-mono text-white/20 uppercase tracking-widest text-center">
